...
# 设置页面标题
st.title("📄 公假单生成工具")
st.write("自动处理Excel数据，按学院排序生成格式规范的公假单")
...
def create_word_document(df, selected_columns, activity_info):
    # 创建文档
    doc = Document()
    ...
    try:
        ...
        if st.button("生成Word文档") and selected_columns:
            with st.spinner("正在生成文档..."):
                doc = create_word_document(df, selected_columns, activity_info)
                ...
                st.download_button(
                    label="📥 下载Word文档",
                    data=file_stream,
                    file_name=f"公假单_{activity_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx",
